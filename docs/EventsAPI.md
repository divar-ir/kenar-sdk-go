...
> WidgetsGeneralNetworkCallResponse EventsGetEventAction(ctx).EventsGetEventActionRequest(eventsGetEventActionRequest).Execute()
...
func main() {
	eventsGetEventActionRequest := *openapiclient.NewEventsGetEventActionRequest() // EventsGetEventActionRequest | 

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.EventsAPI.EventsGetEventAction(context.Background()).EventsGetEventActionRequest(eventsGetEventActionRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `EventsAPI.EventsGetEventAction``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `EventsGetEventAction`: WidgetsGeneralNetworkCallResponse
	fmt.Fprintf(os.Stdout, "Response from `EventsAPI.EventsGetEventAction`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiEventsGetEventActionRequest struct via the builder pattern
